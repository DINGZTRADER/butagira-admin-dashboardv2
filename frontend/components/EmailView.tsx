...
const ReplyModal: React.FC<{ email: Email, onClose: () => void }> = ({ email, onClose }) => {
    const { addEmailReply } = useData();
    ...
    const handleSend = () => {
        addEmailReply(email.id, replyText);
        onClose();
    };
  ...
  return (
    <div className="flex h-full gap-6">
        {isReplying && selectedEmail && (
            <ReplyModal email={selectedEmail} onClose={() => setIsReplying(false)} />
        )}
        {/* Email List */}
        <div className="w-1/3">
