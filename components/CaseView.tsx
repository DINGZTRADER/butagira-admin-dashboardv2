...
const CaseDetail: React.FC<{
  ...
  setView: (view: ViewState) => void;
}> = ({ caseItem, client, caseDocuments, setView }) => {
...
const CaseList: React.FC<{ cases: Case[], clients: Client[], setView: (view: ViewState) => void }> = ({ cases, clients, setView }) => (
    <Card>
...
export const CaseView: React.FC<CaseViewProps> = ({ cases, clients, documents, selectedCase, setView }) => {
  if (selectedCase) {
    const client = clients.find(c => c.id === selectedCase.clientId);
    const caseDocuments = documents.filter(d => d.caseId === selectedCase.id);
    return <CaseDetail caseItem={selectedCase} client={client} caseDocuments={caseDocuments} setView={setView} />;
  }

  return <CaseList cases={cases} clients={clients} setView={setView} />;
};
